<?php
    ...
    $image = 'imtiyaz.jpg';
    if (!empty($curPageName) && $curPageName == 'free-call-with-imtiyaz.php') {
        $image = 'free-discovery-call.jpg';
    }
?>
<div class="row">
        ...
        <form class="" method="post" action="free-call.php">
            <select type="select" name="service" class="form-control" required>
                ...
                <?php if (!empty(SERVICES)) { foreach (SERVICES as $service => $name) {
                    ?>
                    <option value="<?php echo $service;?>" <?php if ($code == $service) {?>selected<?php }?>><?php echo $name;?></option>
                <?php }}?>
            </select><br />
    ...
    <div class="col-lg-5 col-md-5 align-self-center text-center"><img src="img/about/<?php echo $image;?>" 
